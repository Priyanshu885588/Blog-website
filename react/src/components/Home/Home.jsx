...
export const Home = () => {
  ...
  if (singleBlog) {
    return (
      <SingleBlog singlePost={singleBlog} handleSingleblog={handleSingleblog} />
    );
  }
  return (
    <div className=" dark:text-white w-full h-[calc(100vh-4rem)] text-black flex flex-col justify-between items-center">
      <div className="md:w-1/2 w-11/12 flex flex-col justify-center items-center h-full gap-4 pb-2 md:m-0 m-2">
        ...
        <button
          onClick={() => setSingleBlog(posts[0])}
          className="border rounded-full md:text-sm dark:border-white border-black hover:bg-white hover:text-black transition-all duration-500 text-sm open-sans p-2 pr-5 pl-5"
        >
          Read more
        </button>
      </div>
      <div className="w-full flex bg-gray-800 dark:bg-gray-950 text-white justify-evenly gap-4 min-h-52 items-center text-left p-9 flex-wrap">
        {posts.slice(1).map((post, index) => (
          <div
            key={post._id}
            onClick={() => setSingleBlog(post)}
            className="md:w-48 w-full flex flex-col gap-2  border-b p-2 md:border-none cursor-pointer rounded-lg hover:text-yellow-400"
          >
